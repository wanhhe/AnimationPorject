...
bool operator==(const mat4& a, const mat4& b) {
	for (int i = 0; i < 16; ++i) {
		if (fabsf(a.v[i] - b.v[i]) > MAT4_EPSILON) { // 比较两个矩阵中相同位置的每一个元素
			return false;
		}
	}
	return true;
...
// 矩阵乘法用到的宏，列优先存储，所以要找a的某一列，b的某一行
...
vec3 transformVector(const mat4& m, const vec3& v) {
	return vec3(
		M4V4D(0, v.x, v.y, v.z, 0.0f), // 向量的w分量为0
		M4V4D(1, v.x, v.y, v.z, 0.0f),
		M4V4D(2, v.x, v.y, v.z, 0.0f) // 矩阵最后一行是[0,0,0,1]，由于返回的是三维向量故不需要最后一行相乘。最后一行乘出来本来就只会是1(向量)
	);
}

vec3 transformPoint(const mat4& m, const vec3& v) {
	return vec3(
		M4V4D(0, v.x, v.y, v.z, 1.0f), // 点的w分量为0
		M4V4D(1, v.x, v.y, v.z, 1.0f),
		M4V4D(2, v.x, v.y, v.z, 1.0f) // 矩阵最后一行是[0,0,0,1]，由于返回的是三维向量故不需要最后一行相乘。最后一行乘出来本来就只会是0(点)
	);
}

// 用v和w构造四维向量。传递和矩阵相乘后的新w值并返回vec3，只是vec3是由四维产生的。
...
mat4 adjugate(const mat4& m) {
	...
	cofactor.v[1] = -M4_3X3MINOR(1, 2, 3, 0, 2, 3); // 求代数余子式
